...
public:
  ...
  friend bool inspect(Inspector& f, Column& column) {
    auto toBytes = [&column]() -> decltype(auto) {
        return chunkedArray_to_bytes(column.array_);
    };
    auto fromBytes = [&column](const std::vector<std::uint8_t> &bytes) {
        column.array_ = bytes_to_chunkedArray(bytes);
        return true;
    };
...
CAF_BEGIN_TYPE_ID_BLOCK(Column, normal::util::Column_first_custom_type_id)
