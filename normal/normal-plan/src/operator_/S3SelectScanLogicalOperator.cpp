...
//  validPartitions_ = (!predicate_) ? getPartitioningScheme()->partitions() : getValidPartitions(predicate_);
    ...
    auto mode = getMode();
    switch (mode->id()) {
        case plan::operator_::mode::FullPullup:
            return toOperatorsFullPullup(NumRanges);
        case plan::operator_::mode::FullPushdown:
            return toOperatorsFullPushdown(NumRanges);
        case plan::operator_::mode::PullupCaching:
            return toOperatorsPullupCaching(NumRanges);
        case plan::operator_::mode::HybridCaching:
            return toOperatorsHybridCaching(NumRanges);
        case plan::operator_::mode::HybridCachingLast:
            return toOperatorsHybridCachingLast(NumRanges);
        default:
...
//    );
    ...
    for (const auto &partition: *getPartitioningScheme()->partitions()) {
        ...
        if (finalPredicate) {
            ...
            auto predicateColumnNameSet = std::make_shared<std::set<std::string>>(predicateColumnNames->begin(),
                                                                                  predicateColumnNames->end());
...
std::shared_ptr<std::vector<std::shared_ptr<normal::cache::SegmentKey>>>
S3SelectScanLogicalOperator::extractSegmentKeys() {
    auto operators = std::make_shared<std::vector<std::shared_ptr<normal::core::Operator>>>();
    ...
    for (const auto &partition: *getPartitioningScheme()->partitions()) {
        ...
        auto allColumnNameSet = std::make_shared<std::set<std::string>>(projectedColumnNames_->begin(),
                                                                        projectedColumnNames_->end());
        allColumnNameSet->insert(predicateColumnNames->begin(), predicateColumnNames->end());
        auto allColumnNames = std::make_shared<std::vector<std::string>>(allColumnNameSet->begin(),
                                                                         allColumnNameSet->end());
