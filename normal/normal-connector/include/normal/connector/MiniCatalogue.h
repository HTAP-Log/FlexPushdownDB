 ...
 */
    ...
    public:
        ...
        static std::shared_ptr<MiniCatalogue>
        defaultMiniCatalogue(const std::string &s3Bucket, const std::string &schemaName);
        ...
        std::shared_ptr<std::unordered_set<std::shared_ptr<cache::SegmentKey>, cache::SegmentKeyPointerHash, cache::SegmentKeyPointerPredicate>>
    ...
    private:
        std::shared_ptr<std::unordered_map<std::string, std::string>> primaryKeys_;
