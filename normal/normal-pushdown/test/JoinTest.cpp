...
TEST_CASE ("join-test-filescan-join-collate" * doctest::skip(false)) {

  {
	...
	auto joinProbe = std::make_shared<HashJoinProbe>("join-probe",
													 JoinPredicate::create("AA", "BA"), std::set<std::string>{});
	g->put(joinProbe);
...
TEST_CASE ("join-test-non-existent-probe-left-column" * doctest::skip(false)) {
  ...
  // This should fail
  auto probeKernelPutBuildIndexResult = probeKernel.joinBuildTupleSetIndex(tupleSetIndex);
...
TEST_CASE ("join-test-non-existent-probe-right-column" * doctest::skip(false)) {
  ...
  // This should fail
  auto probeKernelPutProbeResult = probeKernel.joinProbeTupleSet(tupleSetA);
...
//  SPDLOG_DEBUG(tupleSetB2->showString(TupleSetShowOptions(TupleSetShowOrientation::RowOriented)));
  ...
  auto probeKernelPutProbeTupleResult1 = probeKernel.joinProbeTupleSet(tupleSetB1);
