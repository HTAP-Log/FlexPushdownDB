...
void simpleSelectRequest(const std::shared_ptr<Aws::S3::S3Client> &s3Client, int index) {
...
#ifdef __AVX2__
    auto parser = std::make_shared<CSVToArrowSIMDChunkParser>(callerName, 16 * 1024 * 1024, inputSchema, outputSchema,
    ...
    while (true) {
        ...
        if (parser->isInitialized()) {
            parser = std::make_shared<CSVToArrowSIMDChunkParser>(callerName, 16 * 1024 * 1024, inputSchema,
                                                                 outputSchema,
                                                                 normal::connector::defaultMiniCatalogue->getCSVFileDelimiter());
        }
...
uint64_t simpleGetRequest(int requestNum) {
    ...
    std::string requestKey = fmt::format(
            "ssb-sf100-sortlineorder/csv_150MB_initial_format/lineorder_sharded/lineorder.tbl.{}", requestNum);
    auto schema = SSBSchema::lineOrder();
    ...
    while (true) {
        ...
        if (getObjectOutcome.IsSuccess()) {
            ...
            SPDLOG_DEBUG("GET request finishes: {}, length: {}, took: {}sec, rate: {}MB/s", requestNum, resultSize,
                         (double) (requestDurationUs) / 1.0e9,
    ...
    auto convertDurationNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            convertStopTime - convertStartTime).count();
...
void normal::ssb::htapTest(int test_mode) {
    ...
    for (const auto &q: queryNames) {
        ...
        // write sql results out for validation
        executeSql(i, sql, false, true, fmt::format("{}output.txt", index));
...
void normal::ssb::mainTest(size_t cacheSize, int modeType, int cachingPolicyType, const std::string &dirPrefix,
    ...
    switch (modeType) {
        case 1:
            ...
            break;
        case 2:
            mode = normal::plan::operator_::mode::Modes::fullPushdownMode();
            modeAlias = "fpd";
            break;
        case 3:
            mode = normal::plan::operator_::mode::Modes::pullupCachingMode();
            modeAlias = "pc";
            break;
        case 4:
            mode = normal::plan::operator_::mode::Modes::hybridCachingMode();
            modeAlias = "hc";
            break;
        default:
            throw std::runtime_error("Mode not found, type: " + std::to_string(modeType));
    }
    ...
    switch (cachingPolicyType) {
        case 1:
            cachingPolicy = LRUCachingPolicy::make(cacheSize, mode);
            cachingPolicyAlias = "lru";
            break;
        case 2:
            cachingPolicy = FBRSCachingPolicy::make(cacheSize, mode);
            cachingPolicyAlias = "lfu";
            break;
        case 3:
            cachingPolicy = WFBRCachingPolicy::make(cacheSize, mode);
            cachingPolicyAlias = "wlfu";
            break;
        case 4:
            cachingPolicy = BeladyCachingPolicy::make(cacheSize, mode);
            cachingPolicyAlias = "bldy";
            break;
        default:
            throw std::runtime_error("CachingPolicy not found, type: " + std::to_string(cachingPolicyType));
    }
...
//  auto sql_file_dir_path = currentPath.append("sql/original");
    ...
    if (cachingPolicy->id() == BELADY) {
        ...
        generateBeladySegmentKeyAndSqlQueryMappings(mode, beladyCachingPolicy, bucket_name, dirPrefix,
                                                    warmBatchSize + executeBatchSize, sql_file_dir_path);
...
void normal::ssb::perfBatchRun(int modeType, const std::string &dirPrefix, int cacheLoadQueries,
    ...
    normal::connector::defaultMiniCatalogue = normal::connector::MiniCatalogue::defaultMiniCatalogue(bucket_name,
                                                                                                     dirPrefix);
