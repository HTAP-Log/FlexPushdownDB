...
void wireUp(std::shared_ptr<normal::plan::operator_::LogicalOperator> &logicalProducer,
    ...
    if (std::find(wiredLogicalProducers->begin(), wiredLogicalProducers->end(), logicalProducer) !=
        wiredLogicalProducers->end()) {
        return;
    }
    ...
    if (logicalProducer->type()->is(operator_::type::OperatorTypes::joinOperatorType())) {
        ...
        if (aggregatePhysicalOperators_pair == logicalToPhysical_map->end()) {
            auto aggregateLogicalOperator = std::static_pointer_cast<normal::plan::operator_::AggregateLogicalOperator>(
                    logicalProducer);
        ...
        if (groupPhysicalOperators_pair == logicalToPhysical_map->end()) {
            auto groupLogicalOperator = std::static_pointer_cast<normal::plan::operator_::GroupLogicalOperator>(
                    logicalProducer);
        ...
        if (projectPhysicalOperators_pair == logicalToPhysical_map->end()) {
            auto projectLogicalOperator = std::static_pointer_cast<normal::plan::operator_::ProjectLogicalOperator>(
                    logicalProducer);
            auto producerOfProjectLogicalOperator = projectLogicalOperator->getProducer();
            wireUp(producerOfProjectLogicalOperator, logicalProducer, logicalToPhysical_map, wiredLogicalProducers,
                   allPhysicalOperators, mode);
        ...
        switch (mode->id()) {
            case normal::plan::operator_::mode::FullPushdown:
                streamOutPhysicalOperators = toPhysicalOperators(logicalProducer, logicalToPhysical_map,
                                                                 allPhysicalOperators);
                break;
            ...
            case normal::plan::operator_::mode::Htap : {
        ...
        throw std::runtime_error(
                "Bad logicalProducer type: '" + logicalProducer->type()->toString() + "', check logical plan");
    }
    ...
    if (logicalConsumer->type()->is(operator_::type::OperatorTypes::joinOperatorType())) {
        ...
        auto joinLogicalOperator = std::static_pointer_cast<normal::plan::operator_::JoinLogicalOperator>(
                logicalConsumer);
        ...
        if (logicalProducer == joinLogicalOperator->getLeftProducer()) {
            ...
            if (joinBuilds->size() > 1) {
                for (const auto &streamOutPhysicalOperator: *streamOutPhysicalOperators) {
                    auto shuffle = normal::pushdown::shuffle::Shuffle::make(
                            streamOutPhysicalOperator->name() + "-shuffle",
                            leftColumnName,
                            Planner::getQueryId());
                    // wire up
        ...
        if (aggregatePhysicalOperators_pair == logicalToPhysical_map->end()) {
            ...
            allPhysicalOperators->insert(allPhysicalOperators->end(), aggregatePhysicalOperators->begin(),
                                         aggregatePhysicalOperators->end());
        } else {
            aggregatePhysicalOperators = aggregatePhysicalOperators_pair->second;
        }
        ...
        if (groupPhysicalOperators_pair == logicalToPhysical_map->end()) {
            ...
            allPhysicalOperators->insert(allPhysicalOperators->end(), groupPhysicalOperators->begin(),
                                         groupPhysicalOperators->end());
        } else {
            groupPhysicalOperators = groupPhysicalOperators_pair->second;
        }
        ...
        if (projectPhysicalOperators_pair == logicalToPhysical_map->end()) {
            ...
            allPhysicalOperators->insert(allPhysicalOperators->end(), projectPhysicalOperators->begin(),
                                         projectPhysicalOperators->end());
        } else {
            projectPhysicalOperators = projectPhysicalOperators_pair->second;
        }
        ...
        auto collatePhysicalOperator = toPhysicalOperators(logicalConsumer, logicalToPhysical_map,
                                                           allPhysicalOperators)->at(0);
        for (const auto &streamOutPhysicalOperator: *streamOutPhysicalOperators) {
        ...
        throw std::runtime_error(
                "Bad logicalConsumer type: '" + logicalConsumer->type()->toString() + "', check logical plan");
    }
